...
def main():
    ...
    for N in range(10, 11, 10):
        ...
        g_real_values = np.arange(0,1.7, 1.6) #np.linspace(0, 3.0, 31)
        M = []
        E_loc=[]
        
        with tf.device('/GPU:0'):
            with open(f'VMC_RBM_energy{N}_transfer.txt', "w+") as file:
                for i, g_real in enumerate(g_real_values[::-1]):
                    if i == 0:
                        ...
                        ## transfer
                        W_array = vmc.W.numpy()
                        bv_array = vmc.bv.numpy()
                        bh_array = vmc.bh.numpy()
                        vmc = VariationalMonteCarloRBM(N,Jz, g_real, g_imag,nh,lr,seed, W_array, bv_array, bh_array)
                    m = []

                    for it in range(steps):
                        samples, _ = vmc.sample(ns, samples)
                        eloc_real, eloc_imag = vmc.training_step(samples, alpha)
                        E_loc.append(np.mean(eloc_real))
                        if it == steps - 1:
